...
def standard_input_OneDay_Hardcoded(gregorian_date = None):
    ...
    info_download =  fnB067_00_setup.hard_setup_02_download()
...
def standard_output_OneDay_Hardcoded(gregorian_date = None):
    ...
    output_folder = fnA99_gf.str_download_output_folder(gregorian_date, fnB067_00_setup)
...
def sp_gen01(sat_prod = None, gregorian_date = None, output_folder = None,  overwrite = False,
                   time_init = "000000", time_fin = "235959"):
    
    datetimeini = str(gregorian_date) + str("-") + str(time_init)
    datetimefin = str(gregorian_date) + str("-") + str(time_fin)
        
   
    GOES.download(Satellite = "goes16", 
                  Product = sat_prod,
                  DateTimeIni = datetimeini, 
                  DateTimeFin = datetimefin, 
                  rename_fmt = '%Y%m%d%H%M%S',
                  path_out = str(output_folder), 
                  retries = 10,
                  backoff = 10,
                  size_format = 'Decimal',
...
def sp_gen02_OneDay_Hardcoded(gregorian_date = None):
    ...
    marco_pila = inspect.currentframe()
    # Obtener el contexto de ejecución actual
    contexto_actual = inspect.getframeinfo(marco_pila)
    
    # Obtener el nombre del módulo al que pertenece la función actual
    nombre_modulo_completo = contexto_actual.filename
    
    # Extraer solo el nombre del módulo (sin la ruta ni la extensión)
    nombre_modulo = inspect.getmodulename(nombre_modulo_completo)
    print(fn01.init_sep()*2)
    ...
    for x in range(len_files):
        ...
        text_out01 = '{} - {} - {} of {}'.format(info_download["sat_name"], 
        ...
        text_out02 = 'Downloading file {}'.format(selected_file)
